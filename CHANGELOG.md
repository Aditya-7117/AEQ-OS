...
- `DOMAINS/live_trading_gate.md` — `LIVE-23..25`, broker/venue documentation compliance: before any task affecting paper or live trading logic, ground the implementation in the project's own uploaded broker/venue documentation rather than assumed or remembered venue behavior; if local docs don't cover it, use a live web search against the venue's official documentation, never a forum post standing in for one; if it still can't be confirmed, stop and ask rather than shipping a guess into paper or live logic. A new, zero-tolerance-tier instance of `CONST-2`'s no-invented-reality standard and `META-15`'s ambiguity discipline, specific to the highest-stakes domain this rulebook covers.
- Total: **454 rules across 24 files**, verified by `scripts/validate.py`.
