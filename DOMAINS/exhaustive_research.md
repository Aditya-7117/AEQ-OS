...
- **RESEARCH-21 — Standard institutional metric suite, reported together, never optimized in isolation.** Every strategy evaluation reports the standard suite computed consistently across the regime (`RESEARCH-3`) and cost (`RESEARCH-4`) bands: Sharpe ratio, Sortino ratio, Calmar ratio, profit factor, CAGR, max drawdown, and win rate. Chasing a single metric in isolation — optimizing for Sharpe alone, say — is a direct path to the exact overfitting `RESEARCH-8` exists to prevent. The suite is reported together specifically so a strategy that's spiked on one metric and mediocre on the rest is visible as such, never hidden behind its best number.
